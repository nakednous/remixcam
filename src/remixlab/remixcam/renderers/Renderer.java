...
public abstract class Renderer implements Renderable, Constants {
	...
	public Renderer(AbstractScene scn, Drawerable dw, boolean hasMatrixStack) {
		scene = scn;
		d = dw;
		if(hasMatrixStack)
  ...
  @Override
	...
	protected void setModelViewMatrix() {
		AbstractScene.showMethodWarning("setModelViewMatrix");
	}
	
	protected void setProjectionMatrix() {
		AbstractScene.showMethodWarning("setProjectionMatrix");
	}
  ...
  public void drawKFIViewport(float scale) {
	...
	public void applyMatrix(Matrix3D m) {
		if( hasMatrixStack() )
			mStack.applyMatrix(m);
		else
			scene.applyMatrix(m);
	}
	
	@Override
  public void applyProjection(Matrix3D m) {
		if( hasMatrixStack() )
			mStack.applyProjection(m);
		else
			scene.applyProjection(m);
	}
  ...
  public Matrix3D getProjection(Matrix3D target) {
		if( hasMatrixStack() )
			...
			return scene.getProjection(target);
  }
